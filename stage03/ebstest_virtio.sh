...
try_dev_stat() {
  ...
  for (( i=0; i < 5; i++ )); do
    ...
    $TIMEOUT 30 ssh -i $instancekey root@$instanceip stat -f $dev
    if (( $? == 0 )); then
      return 0
    fi
  done
